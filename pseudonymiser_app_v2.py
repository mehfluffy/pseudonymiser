...
class ChooseColumns(QtWidgets.QWizardPage):
    ...
    def initializePage(self):
        ...
        options = (  # if modified, need to edit is_completed any(index...) (here) & choiceIdx (Finish)
            "", 
            "Do not use", # skip
            "Copy only", 
            "Pseudonymise"  # hash
    ...
    def isComplete(self):
        ...
        is_completed = all(index >= 1 for index in indexes) and any(index >= 3 for index in indexes)
    ...
    def initializePage(self):
        ...
        for i, colName in enumerate(df.columns):
            if choiceIdx[i] == 3:
                ...
                if self.field("salt") == '':
                    # random salt of random length, different for each column
        ...
        # skip columns
        bools = pd.Series([i == 1 for i in choiceIdx])
        skipIdxs = bools.index[bools]
        df = df.drop(columns = df.columns[skipIdxs])
